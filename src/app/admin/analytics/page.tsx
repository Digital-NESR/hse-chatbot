...
const getAgentName = (_botId: string) => siteConfig.bot.name;
                                                ...
                                                components={{
                                                    ...
                                                    table: ({ children }) => (
                                                        <div className="w-full overflow-x-auto my-4">
                                                            <table className="w-full text-sm text-left border-collapse border border-slate-300 rounded-lg">{children}</table>
                                                        </div>
                                                    ),
                                                    thead: ({ children }) => <thead className="text-xs uppercase text-slate-500 bg-slate-100/50">{children}</thead>,
                                                    th: ({ children }) => <th className="px-4 py-2 font-semibold whitespace-nowrap border-b border-slate-300">{children}</th>,
                                                    td: ({ children }) => <td className="px-4 py-1.5 border-b border-slate-200 last:border-0 whitespace-nowrap">{children}</td>,
                                                    ...
                                                    code: ({ children }) => <code className="bg-slate-200 text-slate-800 rounded px-1 py-0.5 font-mono text-xs">{children}</code>,
                                                    pre: ({ children }) => <pre className="bg-slate-800 text-slate-100 rounded-lg p-4 my-3 overflow-x-auto text-xs font-mono border border-slate-700">{children}</pre>,
                                                    ...
                                                    a: ({ href, children }) => (
                                                        <a href={href} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 underline break-words">
                                                            {children}
                                                        </a>
                                                    ),
                                                }}
                    ...
                    {loadingSessions ? (
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {sessions.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="p-8 text-center text-slate-500">No sessions found.</td>
                                    </tr>
                                ) : (
                                    ...
                                    filteredUsers.map((user) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <td className={`p-4 text-sm hidden md:table-cell ${hasMessages ? 'text-slate-600' : 'text-slate-400'}`}>{user.jobTitle}</td>
                                                <td className={`p-4 text-sm hidden lg:table-cell ${hasMessages ? 'text-slate-600' : 'text-slate-400'}`}>{user.department}</td>
                                                <td className={`p-4 text-sm hidden 2xl:table-cell text-center ${hasMessages ? 'text-slate-600' : 'text-slate-400'}`}>{user.country}</td>
                                                <td className="p-4 hidden sm:table-cell">
                                                    <span className={`px-2 py-1 flex items-center justify-center bg-slate-100 rounded text-xs border border-slate-200 w-fit ${hasMessages ? 'text-slate-700' : 'text-slate-400'}`}>
                                                        {user.totalSessions}
                                                    </span>
                                                </td>
                                                <td className="p-4 hidden sm:table-cell text-center">
                                                    <span className={`text-sm ${user.activeSessions > 0 ? 'text-slate-600 font-medium' : 'text-slate-300'}`}>{user.activeSessions}</span>
                                                </td>
                                                <td className="p-4 hidden sm:table-cell text-center">
                                                    <span className={`text-sm ${user.deletedSessions > 0 ? 'text-red-500 font-medium bg-red-50 px-2 py-1 rounded-md' : 'text-slate-300'}`}>{user.deletedSessions}</span>
                                                </td>
                                                <td className="p-4 hidden sm:table-cell">
                                                ...
                                                <td className={`p-4 text-sm hidden lg:table-cell ${hasMessages ? 'text-slate-600' : 'text-slate-400'}`}>{user.averageMessagesPerSession}</td>
                                                <td className={`p-4 text-sm hidden md:table-cell ${hasMessages ? 'text-slate-500' : 'text-slate-300'}`}>{user.lastActiveDateString}</td>
                                                <td className="p-4 pr-6 text-right">
